...
public class ListActivity extends AppCompatActivity {
    ...
    List<NoticiaModel> mListNoticias;
    ...
    void cagarNoticias(){
        ...
        Call<List<NoticiaModel>> responseCall= ApiClient.getApiClient().getNoticias(idusuario);
        responseCall.enqueue(new Callback<List<NoticiaModel>>() {
            @Override
            public void onResponse(Call<List<NoticiaModel>> call, Response<List<NoticiaModel>> response) {
                if (response.isSuccessful()){
                    List<NoticiaModel> lista = response.body();
                    if (lista!=null){
